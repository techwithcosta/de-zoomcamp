...
for i in range(10):
    ...
    print(f"Sent: {message}")
    time.sleep(0.05)
print(f'\nmessages took {(time.time() - t0_messages):.2f} second')

# flushing
t0_flushing = time.time()
producer.flush()
print(f'flushing took {(time.time() - t0_flushing):.2f} second')

print(f'everything took {(time.time() - t0):.2f} second')

# messages took 0.52 second
# flushing took 0.00 second
# everything took 0.52 second
```

>ANSWER ✅
```
Sending the messages (0.52 second)
```
...
print(f'\nSending took {round(time.time() - t0, 0)} second')
...
>ANSWER ✅
```
Sending 476386 messages took 51.0 second
```
